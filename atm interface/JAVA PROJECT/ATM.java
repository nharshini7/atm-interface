	...
	protected void buildApp(){
		...
		withDraw1.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent event) {
				if(balance >= 100){
					balance = balance - 100;
					displayArea.setText("<html>\u20B9200 Withrawn! <br><br>" + finishedTransaction() + "</html>");
					readyToEnter = false;
					System.out.println("User Has Withdrawn \u20B9200");
					updateTransactionHist("User Has Withdrawn \u20B9200");
				} else {
					displayArea.setText("<html> Your Balance is below \u20B9200. Unable to Withdraw!! <br><br>" 
		...
		withDraw3.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent event) {
				if(balance >= 2000){
					...
					displayArea.setText("<html>\u20B92000 Withrawn! <br><br>" + finishedTransaction() + "</html>");
		...
		withdraw.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent event) {
					displayArea.setText("<html> Withdraw Selected! <br> Please input an amount below and click enter! <br><br>" 
					+ finishedTransaction() + "</html>" );
					readyToEnter = false;
			}	
		});
		...
		enter.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent event) {
				if(readyToEnter == true){
					if(Integer.parseInt(getInputSequence()) > 9999){
						displayArea.setText("That input is greater than \u20B99999!");
						clearInput();
						inputDisplay.setText("Input Display: ");
						readyToEnter = false;
				...
				else if(readyToEnter == false){
					if(Integer.parseInt(getInputSequence()) > balance ){
						displayArea.setText("Input is greater than Current Balance.");
						clearInput();
						inputDisplay.setText("Input Display: ");
						readyToEnter = true;
					}else{
						withdrawBalance(Integer.parseInt(getInputSequence()));
						displayArea.setText("<html> You have withdrawn \u20B9" + getInputSequence() 					
						+ "! <br><br>" + finishedTransaction() + "</html>");
						System.out.println("User Has Withdrawn \u20B9" + getInputSequence());
						updateTransactionHist("User Has Withdrawn \u20B9" + getInputSequence());
					}
					clearInput();
					inputDisplay.setText("Input Display: ");
					readyToEnter = true;
